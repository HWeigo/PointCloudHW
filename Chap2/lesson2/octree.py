...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        ...
        # 屏蔽开始
        root.is_leaf = False
        children_indices = [[] for i in range(8)]
        for pnt_idx in point_indices:
            pnt = db[pnt_idx]
            child_idx = 0
            if pnt[0] > center[0]:
                # child_idx += 1
            ...
            if pnt[2] > center[2]:
                # child_idx += 4
                child_idx |= 4
            children_indices[child_idx].append(pnt_idx)

        # child_extent = extent / 2
        factors = [-0.5, 0.5]
        for i in range(8):
            temp_center = np.zeros(3)
            temp_center[0] = center[0] + extent * factors[(i & 1) != 0]
            temp_center[1] = center[1] + extent * factors[(i & 2) != 0]
            temp_center[2] = center[2] + extent * factors[(i & 4) != 0]
...
def octree_radius_search_fast(root: Octant, db: np.ndarray, result_set: RadiusNNResultSet, query: np.ndarray):
    ...
    if contains(query, result_set.worstDist(), root):
        # query contain current octant, store all the points
